...
```
```{r}
association.rules <- apriori(tr, parameter = list(supp=0.1, conf=0.9,maxlen=10))
...
#flag_same_series(head(select(rules_df, c("lhs", "rhs")),1))
flags_series <- apply(X = select(rules_df, c("lhs", "rhs")), MARGIN = 1, flag_same_series)
#flags_series <- apply(X = head(select(rules_df, c("lhs", "rhs")),50), MARGIN = 1, flag_same_series)
rules_df["is_series"] <- flags_series
