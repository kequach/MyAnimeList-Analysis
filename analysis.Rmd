...
transaction_data <- transaction_data %>% filter(str_count(items, "\\|") <= 300)
rm(transactions_df)
...
flag_same_series <- function(df) {
  ...
  lhs <- str_trim(str_replace_all(tolower(df["lhs"]), "\\{|\\}|\\.|\\!|\\:|\\,", " "))
  rhs <- str_trim(str_replace_all(tolower(df["rhs"]), "\\{|\\}|\\.|\\!|\\:|\\,", " "))
...
get_recommendations <- function(title, tr, supp, conf) {
  rules_rhs <- apriori(tr, parameter = list(supp=supp, conf=conf, maxlen = 2),appearance = list(default="lhs",rhs=title))
  rules_lhs <- apriori(tr, parameter = list(supp=supp, conf=conf, maxlen = 2),appearance = list(default="rhs",lhs=title))
...
```{r}
association.rules <- apriori(tr, parameter = list(supp=0.0001, conf=0.7,maxlen=2))
...
```{r}
single_rules <- get_recommendations("Prison School", tr, 0.0001, 0.7)
single_rules <- get_recommendations("Baki", tr, 0.0001, 0.5)
inspect(single_rules)
# naruto_rules <- apriori(tr, parameter = list(supp=0.0001, conf=0.5, maxlen = 2),appearance = list(default="lhs",rhs="Prison School"))
# 
# rules_df = data.frame(
#   lhs = labels(lhs(naruto_rules)),
#   rhs = labels(rhs(naruto_rules)), 
#   naruto_rules@quality)
# flags_series_naruto <- apply(X = select(rules_df, c("lhs", "rhs")), MARGIN = 1, flag_same_series)
# filtered.rules <- naruto_rules[-which(flags_series_naruto == TRUE, arr.ind=TRUE)]
# inspect(naruto_rules)
# summary(filtered.rules)
```
